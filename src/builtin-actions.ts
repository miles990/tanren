/**
 * Tanren — Built-in Action Handlers
 *
 * The 30+ built-in ActionHandler implementations shipped with Tanren.
 * Infrastructure (parsing, registry, tool definitions) lives in actions.ts.
 */

import { writeFileSync, appendFileSync, mkdirSync, existsSync } from 'node:fs'
import { dirname, resolve, join } from 'node:path'
import type { Action, ActionHandler, ActionContext, RiskTier } from './types.js'
    ...
    async execute(action, context) {
      ...
      if (!url && action.content) {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          else if (/^[A-Za-z][\w-]*:\s/.test(line)) headerLines.push(line)
